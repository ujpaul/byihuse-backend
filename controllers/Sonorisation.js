...
import Rental from "../repos/Sonorisation";
...
exports.getSonorisationRentOrders = async (req, res) => {
  try {
    const type = "sonorisation";
    const results = await Rental.getSonorisationRentOrders(type);
    Response.Success(res, 200, "Orders fetched successfully", results);
  } catch (err) {
    console.log(err);
    Response.InternalServerError(
      res,
